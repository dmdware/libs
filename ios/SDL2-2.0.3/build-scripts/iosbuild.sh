...
#
# Build the armv7s binary
#
prepare_environment "armv7s"
if test x$configure_armv7s = xyes; then
(cd build/armv7s && \
sh ../../configure $CONFIG_FLAGS CC="$CC" CXX="$CXX" CFLAGS="$CFLAGS" LDFLAGS="$LDFLAGS") || exit 2
# configure is not yet fully ready for iOS, some manual patching is required
cp include/* build/armv7s/include
cp include/SDL_config_iphoneos.h build/armv7s/include/SDL_config.h || exit 2
sed -i "" -e "s|^EXTRA_CFLAGS.*|EXTRA_CFLAGS=-I./include|g" build/armv7s/Makefile || exit 2
sed -i "" -e "s|^EXTRA_LDFLAGS.*|EXTRA_LDFLAGS=-lm|g" build/armv7s/Makefile || exit 2
fi
if test x$make_armv7s = xyes; then
(cd build/armv7s && make -j$NJOB) || exit 3
fi
#
# Build the arm64 binary
#
prepare_environment "arm64"
if test x$configure_arm64 = xyes; then
(cd build/arm64 && \
sh ../../configure $CONFIG_FLAGS CC="$CC" CXX="$CXX" CFLAGS="$CFLAGS" LDFLAGS="$LDFLAGS") || exit 2
# configure is not yet fully ready for iOS, some manual patching is required
cp include/* build/arm64/include
cp include/SDL_config_iphoneos.h build/arm64/include/SDL_config.h || exit 2
sed -i "" -e "s|^EXTRA_CFLAGS.*|EXTRA_CFLAGS=-I./include|g" build/arm64/Makefile || exit 2
sed -i "" -e "s|^EXTRA_LDFLAGS.*|EXTRA_LDFLAGS=-lm|g" build/arm64/Makefile || exit 2
fi
if test x$make_arm64 = xyes; then
(cd build/arm64 && make -j$NJOB) || exit 3
fi
